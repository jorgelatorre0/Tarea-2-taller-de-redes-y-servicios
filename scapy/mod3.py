...
if b'success' in resp:
    ...
    enviar(b"<presence type='unavailable'/>")
    print(f"[+] Presence unavailable enviado")
    time.sleep(3)

    payload = b"<message to='usuario2@localhost' type='chat'><body>puedes verme?</body></message>"
    enviar(payload)
    print(f"[+] Mensaje enviado tras presence unavailable")
    resp = recibir()
